...
print(f"{'─'*76}")
print("""
  Purpose: If RF >> LR in MAPE, energy cost is non-linear — confirming
  that structural interactions drive energy, not just individual feature counts.
""")
...
gap     = abs(mape_rf - mape_lr)

print(f"\n  Hypothesis MAPE ≤ 15%:")
print(f"    Random Forest     → {rf_hyp}  ({mape_rf:.2f}%)")
print(f"    Linear Regression → {lr_hyp}  ({mape_lr:.2f}%)")
print(f"\n  Model gap: {gap:.1f} pp — {'confirms non-linearity of energy cost (RF needed)' if gap > 3 else 'small gap'}")
...
print(f"""
  ...
  │  Performance gap: {gap:.1f} pp — {"RF captures non-linear interactions" if gap>3 else "similar performance"}                  │
  └─────────────────────────────────────────────────────────────────────┘
...
print(f"""
  ...
  2. Non-linearity is critical: the {gap:.1f} pp gap between Random Forest
     ({mape_rf:.1f}%) and Linear Regression ({mape_lr:.1f}%) confirms that energy cost
     is NOT an additive function of feature counts. Loop nesting and object
     creation INTERACT — the combination exceeds the sum of their parts.
  ...
  2. Dataset scale: 60 programs is POC; production needs 500+.
  3. Java-only: not yet validated cross-language (Python, C++, Rust).
  4. Busy-wait detection is a proxy, not recursive AST child-count.
  5. No concurrent code: threading/async energy patterns not modelled.
